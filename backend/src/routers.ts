...
import { router, publicProcedure, protectedProcedure, TrpcContext } from "./trpc.js";
import { COOKIE_NAME } from "./auth.js";
import {
  listR7Customers, getR7Customer,
  createR7Customer, updateR7Customer, deleteR7Customer,
} from "./db.js";
  ...
  withoutInc: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const withoutInc = results
        ...
        .map((r) => {
          const { inv, incRef, commentCount } = (r as PromiseFulfilledResult<{ inv: r7.Investigation; hasInc: boolean; incRef: string | null; commentCount: number }>).value;
          return {
            id: inv.id,
            title: inv.title,
            status: inv.status,
    ...
    .query(async ({ ctx, input }) => {
      ...
      const checks = await Promise.allSettled(
        investigations.map(async (inv) => {
          try {
            const commentsData = await r7.getInvestigationComments(c.apiKey, c.region, inv.id);
            const allText = (commentsData?.data ?? []).map((c) => c.body ?? c.content ?? c.text ?? "").join(" ");
...
function analyzeLog(log: r7.LogSource, staleMinutes: number) {
  ...
  const lastReceived = userData["last_received"] ?? userData["lastReceived"] ?? userData["last_event_time"];
